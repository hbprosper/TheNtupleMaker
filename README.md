...
__TheNtupleMaker__ (TNM) is a tool that automates the creation of simple ROOT ntuples from data in the (EDM) format developed and used by the CMS Collaboration. In particular, TNM can be run on CMS mini Analysis Object Data (miniAOD) files.  It also automatically generates a __C++__ and __Python__ analyzer skeleton that can be the foundation of code for analyzing the content of the ntuples. 
