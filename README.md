...
__TheNtupleMaker__ (TNM) is a tool that automates the creation of simple ROOT ntuples from data in the format used by the CMS Collaboration (EDM). In particular, TNM can be run on CMS mini Analysis Object Data (miniAOD) files.  It also automatically generates a __C++__ and __Python__ analyzer skeleton that can be the foundation of code for analyzing the content of the ntuples. 
TNM works with miniAODs built with ROOT 5. Therefore, it is compatible with all versions of CMSSW (https://github.com/cms-sw/cmssw), the CMS Collaboration's codebase, which depends on ROOT 5.  A version of TNM that works with [ROOT 6](http://root.cern.ch) is under development.

TNM is the brainchild of Sezen Sekmen who, in December 2009, argued that in spite of the complexity of typical data formats used by collaborations such as CMS, the data that are ultimately used in a physics analysis are simply a collection of numbers each of which is defined by an access function that returns a simple type, typically, a floating point number or an integer. Therefore, argued Sekmen, it ought to be possible to build a tool that allows the user to create the desired combination of the these data. In CMSSW these access functions are packaged into C++ classes and number in the thousands. TNM is the first realization of Sekmen's vision of a particle physics analysis ecosystem that dramatical reduces the learning curve for those who wish to access, for example, CMS [open data](http://opendata.cern.ch/). The package was developed by Harrison Prosper and Sezen Sekmen with help from Jeff Haas. The ultimate goal is to create an online portal, with something like TNM as a backend, in which access to particle physics data amounts making a query about what data are available, selecting them, pressing a button to create an ntuple that can be transparently accessed using ROOT or whatever it evolves into.

## Installation <a name="installation"></a>

We provide instructions for installing TNM within a [docker](https://www.docker.com/) container (a secure, consistent, software environment isolated from the host), specifically one that provides access to the CMS [open data](http://opendata.cern.ch/) consistent with CMSSW_5_3_32. (Note that TNM can be used with any version of CMSSW built using ROOT 5.)
The instructions are for a Mac, which in addition to __docker__ requires an installation of __[XQuartz](https://www.xquartz.org/)__ so that apps with graphical user interfaces, such as __root__, can be used within a docker container (that is, to provide X11 forwarding).
